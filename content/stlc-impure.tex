...
The axiom can be justified as follows. Suppose following typing
relation holds, what impure variables can be captured by $t_1$?
...
The function passed to \emph{map} in \emph{squareImpure} is not stoic,
as it captures the capability $c$. The function \emph{squareImpure}
has the type $List[Int] \to IO \to List[Int]$, and \emph{squarePure}
has the type $List[Int] \to List[Int]$. All effects are captured in
the type system even in the presence of (local) free functions.
