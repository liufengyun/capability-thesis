...
capsafe/caprod in STLC-Pure with libertine function types.

\subsubsection{Formulation}

With the presence of libertine functions, the previous formulation of
...
libertine function type. However, this formulation has no hope to be
proved, as $S \to T$ is a subtype of $S \Rightarrow T$, any term that
can be typed as the former can also be typed as the latter.

Now let's consider how to define caprod/capsafe for libertine function
...
it would allow calling libertine functions in healthy environments,
...
don't make much difference. Thus, libertine functions types should be
...
rejected by CP-Fun2 is inhabitable, as libertine functions can capture
any capabilities in scope. However, as all libertine function types
...
$(A \Rightarrow B) \to List[A] \Rightarrow List[B]$. When supplied
with a pure function $f$ of type $A \to B$, \emph{map f} has the type
$List [A] \to List[B]$. This is a good news for programmers in want to
effect polymorphism.
