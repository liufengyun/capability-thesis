...
$S \Rightarrow T$ are not subtype of the pure Top. The subtyping
...
know what side effects there might be inside free functions. If we
allow stoic functions to have access to free functions, we'll loose
...
in order to call this function. However, from the perspective of
practicity, it's not useful to have functions in the environment if
they can never be called. Thus, to simplify the system without
sacrificing usability, we removed all free function types from the
pure environment.

\subsection{Soundness}

We proved both progress and preservation in Coq.
...
We restrict $s$ to be a value in the lemma
\emph{Substitution-Not-Top}, for the same reason as we've seen in the
system STLC-Pure, thus we don't reiterate here.
...
consequence of this formulation, we need sligtly update the definition
of the second statement of effect safety:
...
\emph{Why this formulation of healthy environment is acceptable?} In
short, because the statement \emph{Effect-Safety-1} and
\emph{Effect-Safety-2'} logically imply the statement
\emph{Effect-Safety-Inhabitability-1} and
\emph{Effect-Safety-Inhabitability-2} respectively. Therefore, it
suffices to prove the statement \emph{Effect-Safety-1} and
...
function $f$ of type $A \Rightarrow B$ with side effects, \emph{map f}
has the type $List[A] \Rightarrow List[B]$, thus is impure. When
supplied with a pure function $f$ of type $A \to B$, \emph{map f} can
be typed as $List[A] \to List[B]$ according to the axiom
\textsc{Ax-Poly}, thus is pure.
...
effects. It straight-forward to add a new interface to allow exception
effects without changing the implementation.
