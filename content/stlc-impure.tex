...
effect systems, namely axiom-based polymorphism, curry polymorphism
and stoic polymorphism. The first one depends on the axiom
\textsc{Ax-Poly}, while the other two are inherent in capability-based
effect systems.

\subsubsection{Axiom-Based Polymorphism}

Axiom-based polymorphism is based on the axiom \textsc{Ax-Poly}. It
...
\begin{lstlisting}[language=Scala]
  ...
  def map[A,B](f: A -> B)(l: List[A]) = mapImpl(f)(l)
\end{lstlisting}

Note that the only change we made compared to the previous example is
to add the parameter $l: List[A]$ explicitly to the second \emph{map},
instead of using currying. This seemingly trivial change makes a big
difference in capability-based effect systems. Now the second
\emph{map} can be typed as follows $(A \to B) \to List[A] \to List[B]$
...
$(Int \Rightarrow Int) \to Int$. Just checking the type signature of
\emph{twice}, we know it might have side effects, if the passed
function \emph{f} has side effects. Without any annotations or
...
signature of effect-polymorphic functions by using effect type
variables.
