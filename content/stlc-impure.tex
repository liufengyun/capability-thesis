...
replace it with the value \emph{top}. There is no need to consider the
case that the parameter type $T$ is like $B \to Top$ or
...
on two more general statements about \emph{healthy environments}. If
we can arrive at such a definition of \emph{healthy environment} that
a pure and inhabitable environment is also healthy, then it suffices
to prove following two statements about healthy environments:
