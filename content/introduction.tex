...
will explain, monad-based effect systems only work in lazy evaluation
languages; it is unsuitable for Scala because Scala is
...
capability variables in a stoic function will generate an error:
...
\emph{map}s must be pure or only have IO effects respecitvely. The
type system ensures that the first \emph{map} is typed as pure while
the second \emph{map} can only have IO effects, and both are stoic
functions. It's straight-forward to add new \emph{map} interfaces to
allow more kinds of exceptions without changing the implementation. If
the liberary author doesn't care what effects there might be, he can
expose the function \emph{mapImpl} directly to users.  We'll see
details of effect polymorphism in the system STLC-Impure in Section
...
\item Proposed an approach to solve the problem of \emph{effect
  ...
  \ref{sec:effect-polymorphism}). The solution is much simpler and
  more elegant than in type-and-effect systems and monad-based effect
  systems.
\end{itemize}
