...
\emph{Are capsafe types inhabited?} The answer is not necessarily. The
type $E \to B \to E$ is capsafe but uninhabited. Allowing this type in
the capsafe environment doesn't enable us to construct a term of the
capability type $E$.

\emph{Are inhabited types capsafe?} The answer is yes, except the
...
informal proof, the core idea that if $\Gamma$ is pure and inhabited,
and $\Gamma \vdash t : E$, then we can ``collapse'' $\Gamma$ to
$\{b : B\}$ through substitution of the witnesses of the pure and
inhabited types in $\Gamma$.
...
can only be $b$, as $U$ is a pure type and the variable $e$ binds to
the impure capbility type $E$. If $u$ is a function, we have
$b:B \vdash u: U$, as in the typing of stoic functions the rule
