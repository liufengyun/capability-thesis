...
there exists a value $u$ with $\{b:B, \; e:E\} \vdash u: U$. As $u$ is
a value, it can be either a variable or a function. If $u$ is a
...
have $\{b:B\} \vdash u: U$, as in the typing of stoic functions the
rule \textsc{T-Abs} will remove the binding $e:E$ from the
environment. In both cases, we have $\{b:B\} \vdash u: U$. Now using
the substitution lemma, we have:
